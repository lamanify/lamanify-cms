...
export function PatientMedicalTab({ patient, onSave }: PatientMedicalTabProps) {
  ...
  const fetchMedicalRecords = async () => {
    ...
    setLoading(true);
    try {
      // Fetch visits with doctor information and consultation notes
      const { data: visits, error } = await supabase
        ...
        .select(`
          ...
          consultation_notes!consultation_notes_appointment_id_fkey(
            diagnosis,
            symptoms,
            treatment_plan,
            prescriptions,
      ...
      const records: MedicalRecord[] = visits?.map(visit => {
        const doctorProfile = Array.isArray(visit.profiles) ? visit.profiles[0] : visit.profiles;
        const doctorName = doctorProfile 
          ? `Dr. ${doctorProfile.first_name} ${doctorProfile.last_name}`
          : 'Unknown Doctor';

        // Extract diagnosis and notes from session data or consultation notes
        let diagnosis = '';
        ...
        // Also check consultation_notes table
        if (visit.consultation_notes && Array.isArray(visit.consultation_notes) && visit.consultation_notes.length > 0) {
          const latestNote = visit.consultation_notes[0] as any;
          if (!diagnosis) diagnosis = latestNote?.diagnosis || '';
          if (!consultation_notes) consultation_notes = latestNote?.symptoms || latestNote?.chief_complaint || '';
        }
