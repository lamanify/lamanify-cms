-- Phase 4: Access Control & Workflow Optimization Database Foundation
...
CREATE TABLE public.po_templates (
  ...
  department TEXT,
  template_data JSONB NOT NULL DEFAULT '{}', -- Stores template items, terms, etc.
  auto_generate_frequency TEXT, -- 'weekly', 'monthly', 'quarterly', null for manual
  auto_generate_day INTEGER, -- Day of week/month for auto generation
...
CREATE TABLE public.reorder_suggestions (
  ...
  reason TEXT, -- 'low_stock', 'consumption_trend', 'seasonal_demand'
...
CREATE TABLE public.supplier_catalog_items (
  ...
  lead_time_days INTEGER DEFAULT 7,
  catalog_metadata JSONB DEFAULT '{}', -- Store additional catalog info
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
...
BEGIN
  ...
  -- Generate suggestions for medications with low stock
  FOR med_record IN 
    ...
    FROM medications m
    WHERE m.stock_level <= 50 -- Low stock threshold
      AND m.stock_level >= 0
  LOOP
    -- Calculate suggested quantity (30 days supply + buffer)
    IF med_record.daily_consumption > 0 THEN
      suggested_qty := CEIL(med_record.daily_consumption * 45); -- 45 days supply
    ELSE
      suggested_qty := GREATEST(100, med_record.stock_level * 2); -- Minimum or double current
    END IF;
    ...
    INSERT INTO reorder_suggestions (
      ...
      med_record.id,
      suggested_qty,
      med_record.stock_level,
      CASE 
        WHEN med_record.daily_consumption > 0 THEN CEIL(med_record.daily_consumption * 7)
        ELSE 50
      END,
      med_record.daily_consumption,
      priority_level,
      CASE 
        WHEN med_record.stock_level = 0 THEN 'out_of_stock'
        ELSE 'low_stock'
      END,
