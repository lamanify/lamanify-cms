...
CREATE POLICY "Doctors can update their own consultation notes" ON public.consultation_notes
    FOR UPDATE TO authenticated USING (
        doctor_id = auth.uid() OR get_user_role() = 'admin'
    );
...
CREATE POLICY "Staff can create billing" ON public.billing
    FOR INSERT TO authenticated WITH CHECK (
        get_user_role() IN ('admin', 'doctor', 'nurse', 'receptionist')
    );

CREATE POLICY "Staff can update billing" ON public.billing
    FOR UPDATE TO authenticated USING (
        get_user_role() IN ('admin', 'doctor', 'nurse', 'receptionist')
    );
...
-- Generate unique invoice numbers function
